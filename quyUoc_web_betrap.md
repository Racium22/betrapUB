...
# V. QUY TẮC CHÚ THÍCH CODE (CODE COMMENTING RULES)
## 1. Quy tắc chú thích chung

Toàn bộ HTML, CSS và JavaScript phải được chú thích rõ ràng để dễ bảo trì, dễ đọc và dễ nâng cấp về sau.

Mỗi khối giao diện, chức năng hoặc đoạn xử lý logic đều phải có chú thích mô tả công dụng trước khi viết code.

Quy tắc bắt buộc:

Dòng trên là chú thích tiếng Việt.
Dòng dưới mới là code.
Tuyệt đối không viết chú thích bên cạnh hoặc cuối dòng code.
Không viết code rồi mới chú thích phía dưới.
Không viết chú thích quá ngắn hoặc không rõ nghĩa.

Ví dụ đúng:
<!-- Khối menu điều hướng chính của website -->
<nav class="thanh-menu-dieu-huong">
</nav>
Ví dụ sai:
<nav class="thanh-menu-dieu-huong"></nav> <!-- menu -->
## 2.Quy tắc chú thích theo từng loại ngôn ngữ
### HTML
Phải chú thích:

Công dụng của từng section
Ý nghĩa của từng khối giao diện
Vai trò của form, button, menu, card, popup,...
Các khu vực chứa ảnh, slider, banner, footer,...

Ví dụ:
<!-- Khối banner giới thiệu dịch vụ nổi bật -->
<section class="banner-dich-vu">
</section>
...
Khu vực giao diện đang định dạng
Nhóm thuộc tính dùng để xử lý gì
Các hiệu ứng hover, animation, responsive,...

Ví dụ:
/* Định dạng khung card dịch vụ */
.the-dich-vu{
    width: 300px;
    border-radius: 12px;
}

/* Hiệu ứng phóng to khi di chuột vào card */
.the-dich-vu:hover{
...
Chức năng xử lý sự kiện
Công dụng của hàm
Ý nghĩa của biến quan trọng
Các thao tác DOM
Các hiệu ứng popup, slider, filter,...

Ví dụ:
// Hàm mở popup xem chi tiết mẫu tráp
function moPopup(){
    popup.style.display = "flex";
}
## 3.Quy tắc giải thích kỹ thuật

Khi giải thích code hoặc công nghệ:

Sử dụng thuật ngữ kỹ thuật đúng chuyên môn.
Giải thích ngắn gọn, rõ chức năng thực tế.
Ưu tiên mô tả cơ chế hoạt động kỹ thuật.

Cấm:

Dùng ví dụ nhân hóa không cần thiết.
Dùng ngôn ngữ hình ảnh quá đà.
So sánh kiểu: “bộ não”, “đầu bếp”, “ông quản gia”, “người gác cổng”,...

Chỉ sử dụng cách giải thích hình tượng khi có yêu cầu trực tiếp.